...
import { useOrderDetails, useSelfAssignOrder, useSelfDropOrder, useUpdateOrderStatus } from '@/services/queries/orders'
...
const OrderDetails = () => {
    ...
    const handlePickup = () => {
        Alert.alert('Confirm Pickup', 'Are you sure you want to pick up this order?', [
        ...
        Alert.alert('Confirm Drop', 'Are you sure you want to drop this order?', [
            { text: 'Cancel', style: 'cancel' },
            {
                text: 'Drop', style: 'destructive', onPress: () => {
                    selfDrop(order._id, {
                        onSuccess: () => {
                            Alert.alert('Success', 'Order dropped successfully')
                        },
                        onError: (error: any) => {
                            Alert.alert('Error', error.response?.data?.message || 'Failed to drop order')
                        }
                    })
                }
            }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <Header title="Order Details" showBack={true} onBackPress={() => router.back()} />
            <ScrollView contentContainerStyle={{ padding: SPACING.screenPadding, paddingBottom: 50 }}>
                <View style={[styles.card, {marginBottom: 20}]}>
                    <DetailRow label="Order ID" value={order.orderId} />
                    <DetailRow label="Date" value={new Date(order.orderDate).toLocaleDateString()} />
                    <DetailRow label="Customer" value={order.userDetails?.userName || "N/A"} />
                    <DetailRow label="Address" value={order.kycDetails?.location || "N/A"} />
                    <DetailRow label="Product" value={order.productDetails?.productName || "N/A"} />
                    <DetailRow label="Warehouse" value={order.warehouseDetails?.shop_name || "N/A"} />
                    <DetailRow label="Quantity" value={order.quantity} />
                    <DetailRow label="Sale Price" value={`${order.price} AED`} />
                    <DetailRow label="Total Payable" value={`${order.totalPrice} AED`} />
                    <DetailRow label="Payment Method" value={order.paymentStatus === 'unpaid' ? 'Cash/Card on Delivery' : 'Paid'} />
                ...
                {order.agentStatus === 'new' && (
                    ...
                    >
                        {isAssigning ? <ActivityIndicator color="white" /> : <Text style={styles.buttonText}>Pick Up Order</Text>}
                    </TouchableOpacity>
                )}

                {(order.agentStatus === 'assigned' || order.agentStatus === 'dupdated') && (
                    <View style={styles.actionContainer}>
                        ...
                        <View style={styles.buttonGroup}>
                            <TouchableOpacity
                                ...
                                disabled={isUpdating}
                            >
                                {isUpdating ? <ActivityIndicator color="white" /> : <Text style={styles.buttonText}>Update Details</Text>}
                            </TouchableOpacity>

                            <TouchableOpacity
                                style={[styles.button, styles.dropButton, (isDropping || order.agentStatus === 'dupdated') && { opacity: 0.5 }]}
                                onPress={handleDrop}
                                disabled={isDropping || order.agentStatus === 'dupdated'}
                            >
                                {isDropping ? <ActivityIndicator color="white" /> : <Text style={styles.buttonText}>Drop Order</Text>}
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
