...
import { getSalesTarget } from '@/services/api/target';
import { TargetData } from '@/services/api/types/target';
...
const PerformanceSummary = () => {
    ...
    const achievementColor = data ? getAchievementColor(data.targetSummery.totalAchievement, data.targetSummery.totalTarget) : theme.primary;
    const percentage = data && data.targetSummery.totalTarget > 0 
        ? (data.targetSummery.totalAchievement / data.targetSummery.totalTarget) * 100 
        : 0;
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerTitleContainer}>
                    <Text style={[styles.headerSubtitle, { color: theme.primary }]}>Performance</Text>
                    <Text style={[styles.headerTitle, { color: theme.primary }]}>Summary</Text>
                </View>
            ...
            <View style={styles.dateSelector}>
                <TouchableOpacity onPress={() => changeMonth(-1)}>
                   <Icon name="Back" size={moderateScale(20)} color="primary" style={{transform:[{rotate:'0deg'}]}} /> 
            ...
            <ScrollView contentContainerStyle={styles.content}>
                {loading ? (
                    <ActivityIndicator size="large" color={theme.highlightPrimary} style={{marginTop: 50}} />
                ) : (
                    <>
                        {/* Gauge Card */}
                        <View style={styles.card}>
                                ...
                                >
                                    {(fill) => (
                                        <View style={styles.gaugeCenter}>
                                            <Text style={[styles.gaugeStatus, { color: theme.primary }]}>
                            ...
                            <View style={styles.barsContainer}>
                                <View style={styles.barGroup}>
                                    <Text style={[styles.barLabel, { color: theme.primary }]}>Orders</Text>
                                    <View style={styles.progressBarBg}>
                                        <View style={[styles.progressBarFill, { 
                                            width: `${data && data.orderResult.pending + data.orderResult.completed > 0 
                                                ? (data.orderResult.completed / (data.orderResult.pending + data.orderResult.completed)) * 100 
                                                : 0}%`,
                                            backgroundColor: '#00B96F' 
                                        }]} />
                                    </View>
                                    <Text style={styles.barValue}>{data?.orderResult.completed}/{data ? data.orderResult.pending + data.orderResult.completed : 0}</Text>
                                </View>
                                <View style={styles.barGroup}>
                                    <Text style={[styles.barLabel, { color: theme.primary }]}>Warranty</Text>
                                    <View style={styles.progressBarBg}>
                                        <View style={[styles.progressBarFill, { 
                                            width: `${data && data.claimResult.pending + data.claimResult.completed > 0
                                                ? (data.claimResult.completed / (data.claimResult.pending + data.claimResult.completed)) * 100
                                                : 0}%`,
                                            backgroundColor: '#FF0F0F' // Using red as per 'Warranty' typically might need attention or just coloring distinctively
                                        }]} />
                                    </View>
                                    <Text style={styles.barValue}>{data?.claimResult.completed}/{data ? data.claimResult.pending + data.claimResult.completed : 0}</Text>
                                </View>
                            </View>
                        </View>

                        {/* History Section Title */}
                        <Text style={[styles.sectionTitle, { color: theme.primary }]}>History</Text>
                        ...
                        <View style={styles.tabContainer}>
                            ...
                            <TouchableOpacity style={styles.tabButton}>
                                <Text style={styles.inactiveTabText}>Warranty</Text>
                            </TouchableOpacity>
                        ...
                        <View style={styles.listContainer}>
                             <Text style={{textAlign:'center', color:'#888', marginTop: 20}}>
                                 Detailed history list integration pending API support.
                             </Text>
                        </View>
                    </>
                )}
